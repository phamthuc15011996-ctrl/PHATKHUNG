...
let players=[],clients=[],state={phase:"lobby",count:null,results:{}};
const emit=()=>{let m=`data: ${JSON.stringify({players,state})}\n\n`;clients.forEach(c=>c.write(m))};
...
http.createServer(async(req,res)=>{
 ...
 if(u.pathname==="/events"){res.writeHead(200,{"Content-Type":"text/event-stream","Cache-Control":"no-cache","Connection":"keep-alive"});res.write(`data: ${JSON.stringify({players,state})}\n\n`);clients.push(res);req.on("close",()=>clients=clients.filter(x=>x!==res));return}
 if(req.method==="POST"){
  ...
  if(u.pathname==="/clear"){players=[];state={phase:"lobby",count:null,results:{}}}
